    ...
    kriging.plot = function (canvas, grid, xlim, ylim, colors) {
        ...
        for (i = 0; i < n; i++)
            for (j = 0; j < m; j++) {
                ...
                x =
                    (canvas.width * (i * grid.width + grid.xlim[0] - xlim[0])) /
                    range[0];
                y =
                    canvas.height *
...
// from some random stack overflow, made color by self
...
var colorMe = function (val) {
    var r = Interpolate(250, 101, val, 50);
    var g = Interpolate(101, 128, val, 50);
    var b = Interpolate(114, 239, val, 50);
    return { r: r, g: g, b: b };
};
// pseudo analysis (fake datas!)
...
var newVal = [];
var predloop = function (xlim, ylim, model) {
    for (var i = xlim[0]; i < xlim[1]; ++i) {
        for (var j = ylim[0]; j < ylim[1]; ++j) {
            var n = kriging.predict(i, j, model);

            newVal.push({ x: i, y: j, v: n });
        }
    }
};
predloop([4, 24], [4, 24], variogram);
...
for (var i = 0; i < newVal.length; ++i) {
    ...
    var x = newVal[i].x;
    var y = newVal[i].y;
    var color = colorMe(newVal[i].v);
    var myCol = "rgb(" + color.r + "," + color.g + "," + color.b + ")";
    var width = "1";
    var height = "1";
    ...
    rect1.setAttributeNS(null, "fill", myCol);
    c.appendChild(rect1);
}
