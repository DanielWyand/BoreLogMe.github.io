...
const datePicker = {
    clickEvent: function (event) {
        ...
        if (event.target.classList.contains("days")) {
            ...
            if (
                /^2\d{3}\/\d{1,2}\/\d{1,2}$/.test(currentDateStr) &&
    ...
    updateCalendar: function () {
        ...
        for (let i = 0; i < 42; i += 7) {
            ...
            for (let j = 0; j < 7; j++) {
                ...
                let styeleClass = "";
                if (dateStr1 != "") {
                    if (new Date(thisDateStr) - new Date(dateStr1) == 0) {
                        console.log("start");
                        styeleClass += " start ";
                    }
                    if (new Date(thisDateStr) - new Date(dateStr2) == 0) {
                        console.log("end");
                        styeleClass += " end ";
                    }
                    if (new Date(dateStr1) < new Date(thisDateStr) && new Date(thisDateStr) < new Date(dateStr2)) {
                        console.log("duration");
                        styeleClass = "duration";
                    }
                }
    ...
    init: function () {
        document.querySelector("div.fillingField input.drillDate").onfocus = () => {
            this.updateCalendar();
        };
        document.querySelector("div.datePicker").onclick = () => {
            this.clickEvent(event);
        };
        document.querySelector("input.selectedYear").onchange = () => {
            this.updateCalendar();
        };
        document.querySelector("input.selectedMonth").onchange = () => {
            this.updateCalendar();
        };
...
const reorder = {
    ...
    handleMouseDown: function (e) {
        ...
        document.onmousemove = (e) => {
            reorder.handleMouseMove(e);
        };
        document.onmouseup = (e) => {
            reorder.handleMouseUp(e);
        };
    },
        ...
        if (this.isDragging) {
            ...
            for (let i = 0; i < this.allUnits.length; i++) {
                if (i !== this.targetIndex) {
                    this.allUnits[i].style.transform = "translateY(0px)";
                }
            }
            ...
            if (this.currentPosIndex > this.targetIndex) {
                ...
                for (let i = 1; i <= needMoveCount; i++) {
                    if (this.targetIndex + i !== this.allUnits.length) {
                        this.allUnits[this.targetIndex + i] ? (this.allUnits[this.targetIndex + i].style.transform = `translateY(-${moveHeight}px)`) : "";
                    }
                }
...
const logSelector = {
    ...
    init() {
        ...
        this.logNameInput.onkeyup = (e) => {
            if (e.keyCode == 13) {
                currentLog.disabled = true;
            }
        };
        this.logNameInput.onblur = () => {
            currentLog.disabled = true;
        };
...
const infoManger = {
    ...
    listenEvents(fillingField) {
        ...
        emptyInput.onmousedown = (e) => {
            e.preventDefault();
        };
        emptyInput.onclick = (e) => {
        ...
        inputField.onblur = (e) => {
            if (e.target.value !== "") {
                ...
                switch (e.target.className) {
                    case "drillDate":
                        if (/^2\d{3}\/\d{1,2}\/\d{1,2}$/.test(value) || /^2\d{3}\/\d{1,2}\/\d{1,2}~2\d{3}\/\d{1,2}\/\d{1,2}$/.test(value)) {
                            e.target.setAttribute("validation", "valid");
                            break;
                        } else {
                            e.target.setAttribute("validation", "invalid");
                            break;
                        }
                    case "bentoniteSealingDepth":
                        if (parseFloat(value) <= parseFloat(receiver.screenIntervalFrom) || receiver.screenIntervalFrom == "NULL") {
                            e.target.removeAttribute("validation");
                            break;
                        } else {
                            e.target.setAttribute("validation", "invalid");
                            break;
                    ...
                    case "screenIntervalFrom":
                        if (
                            (parseFloat(value) <= parseFloat(receiver.screenIntervalTo) && parseFloat(value) >= parseFloat(receiver.bentoniteSealingDepth)) ||
                    ...
                    case "screenIntervalTo":
                        if (
                            (value <= parseFloat(receiver.totalWellDepth) && value >= parseFloat(receiver.screenIntervalFrom)) ||
                    ...
                    case "totalWellDepth":
                        if (value >= parseFloat(receiver.screenIntervalTo) || receiver.screenIntervalTo == "NULL") {
                            e.target.removeAttribute("validation");
                            break;
                        } else {
                            e.target.setAttribute("validation", "invalid");
...
const soilDescriber = {
    ...
    init() {
        this.addLineButton.onclick = () => {
            this.addLine(null, null, null);
        };
    ...
    validator() {
        this.container.querySelectorAll(".soilProfile").forEach((soilprofileDiv, index) => {
            ...
            if (soilprofileDiv.previousElementSibling && depthFrom.value != soilprofileDiv.previousElementSibling.querySelector("input.depthTo").value) {
                console.log("3");
                depthFrom.setAttribute("validation", "invalid");
            }

            if (soilprofileDiv.nextElementSibling && depthTo.value != soilprofileDiv.nextElementSibling.querySelector("input.depthFrom").value) {
                console.log("4");
                depthTo.setAttribute("validation", "invalid");
            }
        });
    ...
    listenEvents(soilprofileDiv) {
        ...
        moreDiv.onmousedown = (e) => {
            reorder.handleMouseDown(e);
        };
        moreDiv.onblur = (e) => {
