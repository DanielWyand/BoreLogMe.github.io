    ...
    template: {
        ...
        subsurfaceProfile: [
            ...
            [
                ...
                "6.00",
                "Silty Clay",
                "greyish brown to dark grey, mainly silty clay, with some clayey silt and/or sand, wet to saturated, soft, medium plasticity, meidum dilatancy. No stain or odor noted.",
            ],
        ],
        soilSample: [
...
class Log {
    ...
    updateBorders() {
        ...
        if (svg_log.querySelector("g.borderLine")) {
            svg_log.querySelector("g.borderLine").remove();
        }
    ...
    addColTitle() {
        ...
        if (svg_log.querySelector("g.colTitle")) {
            svg_log.querySelector("g.colTitle").remove();
        }
    ...
    updateLogo() {
        ...
        if (svg_log.querySelector("g.companyLogo")) {
            svg_log.querySelector("g.companyLogo").remove();
        }
    ...
    updateHeader() {
        ...
        if (svg_log.querySelector("g.tableTitle")) {
            svg_log.querySelector("g.tableTitle").remove();
        }
    ...
    updateFooter() {
        ...
        if (svg_log.querySelector("g.legend")) {
            svg_log.querySelector("g.legend").remove();
        }
        ...
        text_note.innerHTML = `
            ...
            <tspan font-weight='bold' x="${text_x2}" y="${text_y4}">Lengend: </tspan>
    ...
    updateAxis() {
        ...
        if (svg_log.querySelector("g.axis")) {
            svg_log.querySelector("g.axis").remove();
        }
    ...
    updateSubsurfaceProfile() {
        let svg_log = document.querySelector(`svg#${this.logId}`);
        if (svg_log.querySelector("g.subsurfaceProfile")) {
            svg_log.querySelector("g.subsurfaceProfile").remove();
        }

        if (this.subsurfaceProfile.length == 0) {
            return 0;
        }
        let GL = config.gridLines;
        let PS = report.pageSetting;
        ...
        let g_note = svg_log.querySelector("g.note");
        if (svg_log.querySelector("g.material")) {
            svg_log.querySelector("g.material").remove();
        }
        ...
        this.subsurfaceProfile.forEach((record, index) => {
            ...
            //material type and legend
            let currentWidth = g_material.getBBox().width;
            if (index % 2) {
                legendy = text_y6;
            } else {
                legendx = currentWidth + (index > 1 ? 20 : 0);
                // + 20*(parseInt((index+1)/2));
                legendy = text_y5;
            }
            g_material.innerHTML += `
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="0.5" opacity='0.99' stroke="black" fill="url(#${patternId})" d=
            "M${legendx},${legendy + 2} l20,0 l0,${-lineheight} l-20,0Z"/>
    ...
    updateSoilSample() {
        if (this.soilSample.length == 0) {
            return 0;
        }
        let svg_log = document.querySelector(`svg#${this.logId}`);
        let GL = config.gridLines;

        if (svg_log.querySelector("g.soilSample")) {
            svg_log.querySelector("g.soilSample").remove();
        }
    ...
    updateWellDetail() {
        if (
            ...
            isNaN(this.bentoniteSealingDepth) ||
            isNaN(this.totalWellDepth) ||
            isNaN(this.screenIntervalFrom) ||
            isNaN(this.screenIntervalTo)
        ) {
            return 0;
        }
        ...
        if (svg_log.querySelector("g.wellDetail")) {
            svg_log.querySelector("g.wellDetail").remove();
        }
        if (!this.wellInstalled) {
            return 0;
        }
    ...
    updateWaterLevel() {
        let svg_log = document.querySelector(`svg#${this.logId}`);
        let GL = config.gridLines;
        if (svg_log.querySelector("g.waterLevel")) {
            svg_log.querySelector("g.waterLevel").remove();
        }
